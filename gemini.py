...
async def cmd_setimg(message: Message, command: CommandObject):
    ...
    if not command.args or "," not in command.args:
        return await message.answer(
            ...
            "Разделы: `main, catalog, profile, myrent, tgacc, help, support, reviews`",
            parse_mode="Markdown",
        )
    try:
        section, url = [x.strip().lower() for x in command.args.split(",", 1)]
        allowed = {"main", "catalog", "profile", "myrent", "tgacc", "help", "support", "reviews"}
...
async def tg_sale_catalog(call: types.CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        "🛒 **Каталог ТГ-аккаунтов на продажу**\n\nВыберите лот:",
        reply_markup=kb.as_markup(),
        parse_mode="Markdown",
    )
